...
void cdecl main()
{
	...
	if (!DosScanEnv(ENVIRONMENT,&bigbuf) && bigbuf[1] == ':' &&
		...
		if (DosMakeNmPipe(PIPE,&hp,
			NP_ACCESS_DUPLEX|NP_NOINHERIT|NP_NOWRITEBEHIND,
			1|NP_NOWAIT|NP_READMODE_BYTE|NP_TYPE_BYTE,
			PIPESIZE,PIPESIZE,1000000000L)) goto error;
...
static void far thread()
{
	...
	while (running) {
		if (err = DosConnectNmPipe(hp)) {
			if (err != ERROR_PIPE_NOT_CONNECTED) goto abort1;
			DosSleep(1000L);
			continue;
		}
		DosSetNmPHandState(hp,NP_READMODE_BYTE|NP_WAIT);
	...
	abort2:
		DosFreeSeg(sel);
	abort1:
		DosSetNmPHandState(hp,NP_READMODE_BYTE|NP_NOWAIT);
